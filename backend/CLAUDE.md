...
| **Step 3 执行（RunExecutor + InfluxDB Backend Listener）** | ✅（2026-04-30，见 §17）|
...
| **BackendListenerConfig 单例模型**（pk=1，admin 配置；与 Step 3 InfluxDB 注入路径未融合，follow-up）| ✅（2026-04-29）|
...
| 压测执行引擎（调 JMeter CLI） | ✅（2026-04-30 RunExecutor 落地）|
| MetricSample 时序写入 | ⚠️（表保留，已切到 InfluxDB；v1.2 可清理）|
| 异步调度（Celery + Redis） | ❌ v1.2（当前 threading.Thread）|
| 认证（JWT） | ❌ 故意不做 |
...
## 17. v1.1 Step 3 执行模块（已落地，2026-04-30）

> 早期规划见 §17.x；本节是**已落地**的实际实现。

### 17.0 总览

- **进程模型**：`threading.Thread(daemon=False)` + `subprocess.Popen` 起 JMeter；`RunExecutor` 实例对每个 run 在 web 进程内独立线程编排。模块级 `RUN_EXECUTORS` dict 让 cancel 能找到对应 executor
- **并发约束**：同 task 串行（`TaskRun.Meta.constraints` 里 `unique_active_run_per_task` 部分唯一约束 + 409 Conflict）；不同 task 不限并发
- **指标采集**：JMeter Backend Listener → InfluxDB v1.x（`POST /write?db=jmeter`）；后端 `services/influxdb.py` 只读查询；MetricSample 表保留作降级路径但不再写入
- **取消语义**：graceful 30s（连 nongui.port 发 `StopTestNow`）→ SIGTERM 10s → SIGKILL；Mac/Linux 走 `os.killpg(getpgid)` 整组杀
- **磁盘归档**：每 run 一目录 `<jmeter_home>/runs/<run_id>/`；保留最新 N=20 个，更老的整目录 gzip 成 `<run_id>.tar.gz`

### 17.1 数据流（已落地）

```
POST /api/performance/tasks/:id/run/
        ↓
TaskRun(run_id=secrets.token_hex(8), status=pre_checking) 入库
        ↓ DB 唯一约束撞 → IntegrityError → 409 Conflict
RunExecutor(run).start() → 子线程：
  1. _pre_check() 5 项 → status=pre_check_failed 终止 / 通过则继续
  2. _build_and_write_run_jmx() = build_run_xml(task, inject_environment_dns=True,
                                               inject_backend_listener=True, run_id=...)
     写到 <jmeter_home>/runs/<run_id>/run.jmx
  3. _allocate_stop_port() 从 4445 起按 run_id hash 找空闲端口
  4. status=running, started_at=now, _spawn_jmeter():
       jmeter -n -t run.jmx -l results.jtl -j jmeter.log -e -o report/
              -Jjmeterengine.nongui.port=<stop_port>
  5. _heartbeat_loop():
     - 每 1s 更新 last_heartbeat_at
     - 监听 cancel_requested_at → _send_stoptest() + _wait_or_kill()
     - duration + 60s 超时兜底 → 标 timeout 终态
  6. _on_finish(exit_code):
     - influxdb.query_run_summary(run_id) 拿汇总；失败回退 _summarize_jtl(jtl_path)
     - 终态决策：cancelling→cancelled / timeout→timeout / exit==0→success / 其他→failed
     - cleanup_old_runs(keep=20) 自动归档老目录
```
...
5. **`performance/views.py`**：
   ...
   - 新建 `RunViewSet`（`lookup_field='run_id'` + `lookup_value_regex='[0-9a-fA-F]+'`）含 `retrieve` / `cancel` / `metrics` / `log` / `jtl` / `report`
6. **`performance/serializers.py::TaskSerializer`** 加 `active_run_id` 字段；`get_status` 改成有活跃 run 时直接返 run.status，无 run 时按 `thread_groups_config` 决定 draft/configured
7. **`performance/management/commands/setup_influxdb.py`** —— 幂等建库 + 保留策略
8. **`backend/docker-compose.dev.yml`** —— 起 InfluxDB v1.8 容器，volume 挂 `falcon-influxdb-data`

### 17.3 状态机（TaskRun.status）

```
              ┌─→ pre_check_failed (terminal)
              │
created → pre_checking ─→ pending ─→ running ─┬─→ success
                                                ├─→ failed
                                                ├─→ timeout
                                                ↓
...
`ACTIVE_RUN_STATUSES` = `(pre_checking, pending, running, cancelling)` —— 部分唯一约束的 condition 用它；`is_active` / `is_terminal` properties 暴露给视图层。
